	...
	def assembleIndependent():
		...
		for region in grid.regions:
			...
			for element in region.elements:
				...
				for vertex in element.vertices:
					for coord in range(dimension):
						# print(coord, vertex.getSubElementVolume(element) * (-1) * rho * g[coord])
...
def main_plot():
	...
	for idx,t in zip(stepInTimeSamples,times_1):
		p_a = terza.getPressureValuesConstTime(t)
		p_n = df[f"TimeStep{idx} - p"]
		ax[0][0].plot(p_a*kPa, z, label=f"t = {timeStep*idx}s")
