...
def geomechanics(problemData, initialTime=0.0):
...
def main_run():
	...
	problemData = PyEFVLib.ProblemData(
		...
		outputFilePath = "{RESULTS}/geomechanics/twg",
		numericalSettings = PyEFVLib.NumericalSettings( timeStep = timeStep, maxNumberOfIterations = 5 ),
		propertyData = propertyData,
		boundaryConditions = PyEFVLib.BoundaryConditions(boundaryConditionsDict),
	)

	pField, uxField, uyField = geomechanics( problemData )
	...
	problemData = PyEFVLib.ProblemData(
		meshFilePath = meshFilePath,
		outputFilePath = "{RESULTS}/geomechanics/twg",
		numericalSettings = PyEFVLib.NumericalSettings( timeStep = timeStep, maxNumberOfIterations = 600 ),
		propertyData = propertyData,
		boundaryConditions = PyEFVLib.BoundaryConditions(boundaryConditionsDict),
	)
...
filePath = dirname+"/../../../results/geomechanics/twg/Results.csv"
...
def main_plot():
	...
	stepInTimeSamples = [1, int(0.25*numberOfTimeSteps), int(0.5*numberOfTimeSteps), numberOfTimeSteps-1]

	# print([terza.getPressureValue(y, 0.0) for y in Y])
	...
	for idx,t in zip(stepInTimeSamples,times_1):
		...
		ax[0][0].scatter(p_n*kPa, Y, marker=".",linewidth=0.5)
	...
	for idx,t in zip(stepInTimeSamples,times_1):
		...
		ax[0][1].plot(w*mm, z, label=f"t = {t:.02f}s")
		ax[0][1].scatter(w_n*mm, Y, marker=".",linewidth=0.5)
